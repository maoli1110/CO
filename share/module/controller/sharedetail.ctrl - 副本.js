...
bvShare.controller('sharedetailCtrl', ['$scope','$stateParams','$state','Share','$timeout',
    function ($scope,$stateParams,$state,Share,$timeout) {
	    ...
	    //bv 分享页面图片预览功能start
	    var showImgSignal = false;
		$scope.previewSignEle = function(obj,fileUrl,source,index){
			if(source === 1){
				if(!showImgSignal){
					angular.forEach($scope.collaList.pictures, function(value,key){
						var showImgMSG = '<li style="width: 640px;" class="pic_'+key+'"><img src="'+value.fileUrl+'";></li>';
						$("#scroll_pic_view_ul").append(showImgMSG);
					});
					showImgSignal = true;
				}
				//debugger
		...
		if(isWeiXin()){
			...
			if(isIOS()){
				...
				// location.href = url;
				    ...
				    //     window.location = 'http://down.lubansoft.com/BIM%20View%20V1.0.0%20for%20Android.apk';
				    // }, 500);
			}
		}
	}
