 ...
 * for CO v2.0.0
 */
angular.module('cooperation').controller('editCooperationCtrl', ['$scope', '$http', '$uibModal', '$httpParamSerializer', 'FileUploader', 'Cooperation', 'items', 'Common', 'Manage', '$timeout', 'headerService', '$sce', '$uibModalInstance',

    function ($scope, $http, $uibModal, $httpParamSerializer, FileUploader, Cooperation, items, Common, Manage, $timeout, headerService, $sce, $uibModalInstance) {
        $scope.device = false;
        $scope.flag = {};
        ...
        Cooperation.getCollaboration(coid).then(function (data) {
            //负责人头像路径及名字
            $scope.responsiblePerson.username = data.collaborator;
            $scope.responsiblePerson.avatar = data.collaboratorAvatar;
            angular.forEach(data.relevants, function (value, key) {
                if (value.username == $scope.responsiblePerson.username) {
                    value.mustExist = true;// 编辑时原有的相关人是否必须存在 true必须存在  false可以不存在
                };
                if (value.realname == $scope.responsiblePerson.username) {
                    value.mustExist = true;// 编辑时原有的相关人是否必须存在 true必须存在  false可以不存在
                };
            ...
            $scope.priority = data.priorityValue;
            if ($scope.device) {
         ...
         * @param needSign
         */
        function clearIndex(array,name){
            var index;
        ...
        $scope.deleteRelated = function(index,name,needSign){
            var delIndex;
            $scope.relatedNew.splice(index,1);
            if(needSign){
                 delIndex = clearIndex($scope.related.sign,name)
                 $scope.related.sign.splice(delIndex,1);
            }else if(!needSign){
                 delIndex = clearIndex($scope.related.noSign,name)
                 $scope.related.noSign.splice(clearIndex,1);
            }
        ...
        $scope.ok = function () {
            ...
            if(data.deadline){
                data.deadline = new Date($scope.dt).toLocaleDateString().split('/').join('-')
            }

            Cooperation.updateCollaboration(data).then(function (result, status) {
                var param = {};
                param.coid = data.coid;
        ...
        $scope.selectRelated = function () {
            console.log($scope.related,'弹窗的传值问题')
