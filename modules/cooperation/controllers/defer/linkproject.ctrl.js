...
//			treeObj.expandNode(nodelist[0], true, false, null, true);
		...
		function findAllChilds (data)
		{
			...
			for(var x = 0; x < data.length; x++) {
				...
				}else if(data[x].type == 3 && data[x].value.indexOf("8-3-") == 0 ) {					// 班筑精装
					data[x].iconSkin = "pdsremiz"; //"imgs/icon/8.png";
				}else if(data[x].type == 3 && data[x].value.indexOf("9-3-") == 0 ) {					// 班筑精装
					data[x].iconSkin = "site"; //"imgs/icon/8.png";
		...
		function categoryprojtype(node){
			...
			if(str1=='2'){
				...
				remiznodestore.push(str2);
			}else if(str0=="9"){
				sitenodestore.push(str2);
			}
		}
	 	...
	 	function projTypeSwitch (n) {
	 		switch(n)
				...
				case 6:
				  	...
				  	break;
			 	case 8:
				  	return remiznodestore;
				  	break;
			 	case 9:
				  	return sitenodestore;
				  	break;
				}
