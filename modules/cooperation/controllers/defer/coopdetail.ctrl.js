...
angular.module('cooperation').controller('coopdetailCtrl', ['$scope', '$http', '$uibModal', '$httpParamSerializer', 'FileUploader', 'Cooperation', '$state', '$stateParams', 'Manage', '$sce', '$timeout', 'Common', 'headerService',
    function ($scope, $http, $uibModal, $httpParamSerializer, FileUploader, Cooperation, $state, $stateParams, Manage, $sce, $timeout, Common, headerService) {
                ...
                layer.alert(error.message, {
                    ...
                    if (currentDeptId == -1) {
                        $state.go('cooperation', {
                            'deptId': currentDeptId ? currentDeptId : selfDeptId,
                            'source': rember
                        }, {location: 'replace'});
                    }
                    $state.go('cooperation', {
                ...
                Cooperation.checkOut(coid).then(function (data) {
                    ...
                    modalInstance.result.then(function (param) {
                        if ($scope.device) {
                            ...
                            document.location = '#/coopdetail/?coid=' + param;
                        }
                        console.log('success');
                    }, function () {
        ...
        $scope.updateComment = function () {
            if (!accessCode || (accessCode && accessCode.indexOf(accessCodeConfig.updateCode) == -1)) {
        ...
        $scope.$on('ngRepeatFinished', function (ngRepeatFinishedEvent) {
            ...
            if ($('.common-picture').children().length > 0 && $('.common-files').children().length > 0) {
                $('.common-picture').css({'border-bottom': '1px dashed #ddd', 'padding-bottom': '20px'})
            } else {
                $('.common-picture').css({'border-bottom': 'none', 'padding-bottom': '0px'})
            }
        });
        ...
        $scope.addApprove = function () {
            ...
            Cooperation.checkOut(coid).then(function (data) {
                ...
                modalInstance.result.then(function () {	// 向后台发送需要保存的数据
        ...
        $scope.backCooperation = function () {
            ...
            $state.go('cooperation', {
                'deptId': $scope.collaList.deptId,
                'ppid': $scope.collaList.ppid,
        ...
        $scope.deleteCoop = function () {
            //判断是否有删除权限
            if (!accessCode || (accessCode &&  accessCode.indexOf(accessCodeConfig.deleteCode) == -1)) {
                layer.alert('没有当前功能使用权限，请联系企业管理员', {
                    title: '提示',
                    closeBtn: 0,
                    move: false
        ...
        $scope.ok = function () {
            ...
            $scope.statusCommon = parseInt($scope.statusCommon);
            var data = {
                // coid: items.coid,
...
//签署意见弹框Ctrl
angular.module('cooperation').controller('previewSignCommentCtrl', ['$scope', '$http', '$uibModal', 'items', '$httpParamSerializer', 'FileUploader', 'Cooperation', '$uibModalInstance',
    function ($scope, $http, $uibModal, items, $httpParamSerializer, FileUploader, Cooperation, $uibModalInstance) {
        var currentReact = '17,496,25,896'; //需要给客户端提供的弹框显示区域范围
        var currentEditOfficeUuid = '';
        var currentSuffix = '';
        var progressName;
        $scope.flag = items.flag;
        $scope.isdocPreview = items.isdocPreview;
        $scope.isPriview = items.isPriview;
        console.log($scope.priview,'$scope.priview')
        if ($scope.isdocPreview) {
            progressName = LogConfiguration.progressName.previewSign
        } else {
            progressName = LogConfiguration.progressName.addApprove
        }
        //预览界面跳转回详情
        function backDetail() {
            //没有点击添加审批意见不调用签入
        ...
        if ($scope.isSign) {
            $('.preview_sign_comment').css('top', '-13% ')
        } else {
            $('.preview_sign_comment').css('top', '0% ')
        }
        $scope.backDetail = function (source) {
            if (source == '') {
                if (!BimCo.IsModify()) {
        ...
        //预览界面的签字
        $scope.isPriviewSing = function(){
            Cooperation.checkOut(items.coid).then(function (data) {
                $scope.isPriview = false;
                var pdfSign = BimCo.PdfSign(items.uuid, items.suffix, currentReact, items.coid, Number(items.isSign));
            },function(error){

                $scope.isPriview = true;
                BimCo.MessageBox("提示", error.message, 0x31);
                layer.closeAll();
            });

        }
        //签署意见
        $scope.signComment = function (event) {
            $('.icon-doc-edit>a>span').removeClass('detail-actives');
            $(event.currentTarget).find('span').addClass('detail-actives');
            $scope.isSign = true;
             ...
             */
            console.log($scope.isdocPreview);
            console.log(progressName);

            Cooperation.writeLog(1, '', $scope.flag.LgDetailMode,progressName , 0, '', LogConfiguration.previewSignName.signComment);

        }
        //电子签名
        $scope.signElectronic = function (event) {
            $('.icon-doc-edit>a>span').removeClass('detail-actives');
            $(event.currentTarget).find('span').addClass('detail-actives');
            //获取电子签名
            var signature;
             ...
             * defaultLog不区分两栏、三栏的情况 0：不区分 1：区分
             */
            Cooperation.writeLog(1, '', $scope.flag.LgDetailMode,progressName , 0, '', LogConfiguration.previewSignName.signElectronic);
        ...
        $scope.SubmitAll = function () {
            ...
            if (!BimCo.IsModify()) {
                //pdf是否修改标志
                Cooperation.checkIn(items.coid).then(function (data) {
                });
                $scope.flag.isPdfModify = false;
                $scope.backDetail('submit');
                $uibModalInstance.close(items.flag);
            ...
            if (rtn == 1) {
                ...
                $scope.isChange = true;
                $uibModalInstance.close($scope.isChange);

                Cooperation.writeLog(1, '', $scope.flag.LgDetailMode,progressName , 1, items.name, LogConfiguration.previewSignName.submitAll);
        ...
        $scope.signCancel = function () {
            if (!BimCo.IsModify()) {
                ...
                $uibModalInstance.dismiss('cancel');
                console.log(items.btnType,'type')
                if(items.btnType==2 || !$scope.isPriview ){
                    Cooperation.checkIn(items.coid).then(function (data) {
                    });
                }
        ...
        //清空
        $scope.signEmpty = function (event) {
            $('.icon-doc-edit>a>span').removeClass('detail-actives');
            $(event.currentTarget).find('span').addClass('detail-actives');
            BimCo.SignEmpty();
            Cooperation.writeLog(1, '', $scope.flag.LgDetailMode,progressName , 0, '', LogConfiguration.previewSignName.signEmpty);
