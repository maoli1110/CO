...
angular.module('cooperation').controller('coopreationCtrl', ['$scope', '$http', '$uibModal','$httpParamSerializer','FileUploader','Cooperation','$state','$stateParams','$location','$timeout',
	...
	function createFilterTemplate(item,signal){
		...
		angular.forEach(items,function(value,key){
	    	...
	    	var filterOptionsTemp = '<span type="'+value.type+'">'+value.name+' x</span>';
	    ...
	    if(!queryData.ppid){
	      $scope.cooperationList = []; 
    		Cooperation.getCollaborationList(queryData).then(function (data) {
    			$scope.cooperationList = data;
    			//计算当前协作总条数
				$scope.coItemsTotals = $scope.cooperationList.length;	    
    			if($scope.cooperationList.length <= 0 && queryData.groups.length){
					$scope.coNoResult = true;
					//显示当前搜索无结果
					$scope.searchNoCo = true;
    			} else if($scope.cooperationList.length <= 0 && !queryData.groups.length){
    				$scope.coNoResult = true;
    				//当前无协作
					$scope.noRelatedNoCo = true;
    			}
	...
	var token = false;
	  ...
	  Cooperation.getCollaborationList(queryData).then(function (data) {
		   ...
		   if($scope.cooperationList.length <= 0){
				...
				if(!$scope.cooperationList.length && !queryData.groups){
					//显示当前无结果
					$scope.searchNoCo = true;
				}else if (!$scope.cooperationList.length && queryData.groups) {
					//显示当前工程无协作
					$scope.projNoCo = true;
				}
		   }
    ...
    $scope.changeFilter = function(item,signal){
    	...
    	console.log(queryData.groups+'groups')
    	//将筛选条件放入session
    	addSessionValue();
...
//						blank.name = key+", "+obj.count;
				...
				function echartCenter(){
					return ['60%',document.body.offsetWidth>1380?'66.5%':'50%'];
				}
    ...
    function addSessionValue () {
		...
		sessionStorage.filterSessionOptions = JSON.stringify(filterSessionOptions);
		//当前coopattr,当前搜索主题
	...
	$scope.removeAll = function(){
		...
		layer.confirm('是否删除这'+delItems.length+'条协作吗？', {
			btn: ['是','否'], 
			move: false
		}, function(){
			layer.closeAll();
	...
	$scope.allSelect = function (){
		console.log('checked',$scope.allSelected)
		if($scope.allSelected){
			delItems = [];
