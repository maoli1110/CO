 ...
 * linkbeCtrl
 */
angular.module('cooperation').controller('linkbeCtrl', ['$scope', '$http', '$uibModalInstance','Cooperation','items',
		...
		Cooperation.getDeptList().then(function (data) {
			...
			Cooperation.projectList(deptId).then(function (data) {
				//debugger;
					...
					console.info('haohaohao',$scope.projectList.availableOptions[0]);
					ppid = $scope.projectOption;
					//获取BE资料树
					getDocTagList(ppid);
			});
		});
