 ...
 * 协作管理
 */
angular.module('cooperation').controller('newcoopreationCtrl', ['$scope', '$http', '$uibModal','$httpParamSerializer','FileUploader','Cooperation','$state',
	 ...
	 function ($scope, $http, $uibModal, $uibModalInstance, Cooperation) {
		...
		function zTreeOnClick (event, treeId, treeNode) {
	 	...
	 	$scope.ok = function () {
	 		...
	 		var params = {ppid:ppid, projType:projType}
	 		$uibModalInstance.close();
	 		var modalInstance = $uibModal.open({
				backdrop : 'static',
	    		templateUrl: 'template/cooperation/link_component_category.html',
	    		controller:'openedProjectCtrl',
	    		resolve:{
	    			items: function () {
	    				return params;
	    			}
